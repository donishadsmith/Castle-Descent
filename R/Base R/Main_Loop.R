#Variable used to determine if the player is greeted with the welcome screen or not
iteration <- 0
start_game <- function(){
  ...
  #Add player menus
  player$menus <- menus <- list("genie" = matrix(c("\U2771","Increase Attack","","Reduce Mana Cost"), ncol = 4)
  ...
  while(!(player$encountered_object=="\U2395" & player$floor == player$total_floors | player$hp <= 0 | zombie$distance_to_player == 0)){
    ...
    if(player$zombie_halt > 0){
      new_line(2)
      cat(sprintf("Number of steps left before zombie can move: %s left.",player$zombie_halt))
      }
    new_line(2)
    player_action <- ""
    while(!(player_action %in% c("w","a","s","d","i"))){
      ...
      player_action <- tolower(noquote(readline("w(up), a(left), s(down), d(right), inventory(i): ")))
    }
    if(player_action == "i"){
